...
def ALGO(X_train, components_removed = 4):
...
train=zip(labels_train, train_vectors)
...
val=zip(labels_val, val_vectors)
val_dict=dict(val)
...
test=zip(labels_test, test_vectors)
test_dict=dict(test)
...
yy_v=[]
n=0
for i in val_vectors:
    x=ones((len_v[n],1))*np.array([i])
    yy_v.append(x)
    n=n+1
yy_vl=[]
for i in yy_v:
...
output=layers.Dense(300, activation='linear')(dense_2)
...
plt.title('RMSE_PCA_30_PCA_30_epochs')
plt.show()
plt.plot(history.history['loss'])
plt.title('LOSS_PCA_27_PCA_30_epochs')
plt.show()


pred=[]
for i in X_test:
    y_pred=model_MSE.predict(i)
    pred.append(y_pred)
...
for p in pred:
    ...
    for i in range(0, len(p)):
        cos=[]
        cos=list(cosine_similarity(voc_w2v, p[i].reshape(1,-1)))
        max_=np.max(cos)
    ...
    w2v_=voc_w2v[idx_max]
    w2v.append(w2v_)


words_predicted_df=pd.DataFrame(w2v)
words_predicted_df['words_predicted']=words_predicted
words_predicted_df.to_csv('words_predicted_30_reducted_PCA_30_epochs_4.csv')

#######################################################################

#similarity matrix
#load words predicted 
words_predicted=pd.read_csv('words_predicted_300_PCA_30_epochs_.csv')
...
for i in range(len(human)):
    category.append('human')
for i in range(len(music)):
    category.append('music')
for i in range(len(animal)):
    category.append('animal')
for i in range(len(things)):
    category.append('things')
for i in range(len(nature)):
    category.append('nature')
df=pd.DataFrame(lista_true)
df['category']=category
df.to_csv('lista_true.csv', index=False)
...
for i in range(len(animal_1)):
    category.append('animal')
for i in range(len(things_1)):
    category.append('things')
for i in range(len(nature_1)):
    category.append('nature')
df=pd.DataFrame(lista_predicted_1)
df['category']=category
df['index']=lista_index_1
df.to_csv('lista_predicted.csv', index=False)
...
cm_pred=csm(np.array(lbl_predicted), np.array(lbl_predicted))
...
words_predicted_df=pd.read_csv('words_predicted_30_reducted_PCA_30_epochs_.csv')
words_pred_df=words_predicted_df['0']
...
for i in range(len(words_predicted_df)):
    ...
    r=1-spatial.distance.cosine(test_vectors[i],words_pred_df[i])
    results.append(r)

result=pd.DataFrame(y_test)
result['words_predicted']=words_predicted_df['words_predicted']
result['results']=results
result.to_csv('results_cs_30_reducted_PCA_30_epochs_.csv')
...
df_a_precision_pred_mean['Average_precision_30_reducted_PCA'] = a_precision_pred_mean
df_a_precision_pred_mean.to_csv('Average_precision_30_reducted_PCA.csv')
...
data = {'15_PCA':value_15_PCA , '30_PCA': value_30_PCA, '50_PCA':value_50_PCA ,'70_PCA':value_70_PCA , '100_PCA':value_100_PCA,'180_PCA':value_180_PCA, '300_PCA':value_300_PCA}
